...
To run one line at a time: Ctrl + Enter
To run the whole chunk of codes: Ctrl + Shift + Enter
...
boxplot(big5$Agreeableness)
sd(big5$Agreeableness)
fav_stats(big5$Agreeableness)
...
boxplot(big5$Agreeableness ~ big5$`Sex(M/F)`, col = c(4,1010))
# col = c 
# col: column
# c: color
...
favstats(big5$Conscientiousness ~ big5$`Sex(M/F)`)
```
...
boxplot(big5$Conscientiousness ~ big5$`Sex(M/F)`, col=c(2,4), main="Conscientiousness index among male and female", xlab="Sex", ylab="Conscientiousness index")
