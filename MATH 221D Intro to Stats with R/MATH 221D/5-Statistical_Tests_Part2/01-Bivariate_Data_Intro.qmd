...
__Answer__:  The correlation coefficient, r, quantifies for the relation between 2 quantitative variables.

__Question__:  What does it mean if the correlation coefficient, $r$, is negative?
__Answer__:  If the correlation coefficient is negative, it means these 2 quantitative variables have a negative correlation. For example, as the x value goes up, the y value goes down as return.
...
__Answer__:  The student confidence rating, which is the column 'ConfidenceRatingMean'.

__Question__:  Which is the Response variable, $y$?  
__Answer__: The math exam score, which is the column 'Score'.
...
ggplot(math, aes(x = ConfidenceRatingMean, y = Score )) +
  geom_point(color = "darkblue") + # Use geom_point() function to create point(dot) graph
  theme_bw() +
  labs(
...
__Answer__:  The direction of the correlation between student confidence and the math test score, tends to be positive. The strength of the correlation, isn't as strong as 1 but still good enough to identify the direction.

__Question__:  What's your best guess at, $r$ based on the scatterplot?  
__Answer__:  I will guess 0.5
...
__Answer__:  The correct r is 0.7278648 and my guess is 0.5. So, it's not bad!
...
# GGplot
ggplot(data = geyser, mapping = aes(x = Wait, y = Duration)) +
  geom_point(color = "darkblue") + # Use color to change the color of point (dot)
  theme_bw() + # Remember to put the () behind the theme_bw 
  labs(title = "The Relationship between Waiting Time and The Eruption Duratioin of Yellowstone Geyser")

```

__Question__:  Before calculating the Correlation Coefficient, r, describe in words the direction and strength of the relationship.  
__Answer__:  The direction of the correlation between Wait and Duration tends to be positive. The strength of the relationship is strong enough to recognize it's a positive correlation.

__Question__:  What's your best guess at, $r$ based on the scatterplot?  
__Answer__:  My guess is 0.73
...
__Question__:  How far off was your guess?  
__Answer__:  The correct correlation coefficient, r, is 0.9008112. My guess is 0.73, so it's not bad!
