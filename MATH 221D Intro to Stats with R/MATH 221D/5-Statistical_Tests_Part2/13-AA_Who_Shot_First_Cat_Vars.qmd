...
addmargins(table(sw$`Favorability_Han Solo`, sw$Gender)) # Use addmargins(table()) to get the numbers of data in each different answer categories. order: row, colummn.
# Row: Favorability_Han Solo
# Column: Gender

addmargins(table(sw$Gender, sw$`Favorability_Han Solo`)) # Use addmargins(table()) to get the numbers of data in each different answer categories. 
# Row: Gener
# Column: Favorability_Han Solo
...
-> 110 people out of 834

```{r}
addmargins(table(sw$`Favorability_Emperor Palpatine`)) # Use addmargins(table()) to get the numbers of data counts in each unique answer categories

prop.table(table(sw$`Favorability_Emperor Palpatine`)) # Use prop.table(table()) to get the proportion percentage in each unique answer categories
```
...
```{r}
addmargins(table(sw$`Favorability_Jar Jar Binks`, sw$Gender))
```
