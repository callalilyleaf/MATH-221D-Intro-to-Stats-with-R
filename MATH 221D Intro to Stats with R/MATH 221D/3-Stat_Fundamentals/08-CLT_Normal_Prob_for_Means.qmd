...
The Z-score of :
individual: z = (x - u)/sigma
sample mean: z = (x-bar(sample mean) - u)/sigma/sqrt(n)
...
sd_xbar = sd / sqrt(n) # It's more organized to have: xbar, mu_xbar, sd, and 

# z-score of the mean of sample means in the n samples distribution
z = (xbar - mu_xbar)/sd_xbar 
z

# The probability of having a result above the mean of sample means(27)
1 - pnorm(z)
```
...
__Answer__:  Since the size of samples n = 45 >= 30, the mean of the distriution of the sample means equals to the population mean, which is 3.5
...
# The number of samples. In this case, it's only Dianne. So, n = 1
n <- 1
...
pnorm(160, mu_xbar, sd) - pnorm(145, mu_xbar, sd)

qnorm(.93, mu_xbar, mu_sd) # If the question is asking about a certain percentile score, which the n = 1, we should use the original sd for it
...
__Answer__:  1 - pnorm(z) = 0.1479065
...
__Answer__:  0.9974107

__Question__: What is the GRE score of 93 percentile?
__Answer__: 163.787
