...
```
# Select function deal with columns
...
# Use Ctrl + Shift + M to have "%>%" (the pipe)

```{r}

handed_data <- survey %>%
  select(Handed, Reaction_time) %>% # Select "Handed" and "Reaction_time" columns 
  filter(Reaction_time < 1)

view(handed_data)

favstats(handed_data$Reaction_time ~ handed_data$Handed)

select(survey, Handed, Reaction_time)

boxplot(handed_data$Reaction_time ~ handed_data$Handed, main = "Reaction Times for Different Handness", xlab="Handedness", ylab="Reaction Time", col = c(2,4,6))
