...
favstats(survey$Computer_Use_Hours ~ survey$Gender)

boxplot(survey$Computer_Use_Hours ~ survey$Gender)

histogram(survey$Computer_Use_Hours ~ survey$Gender)
...
```{r}
# the %>% operator throw survey data in to filer function, it's called "pipe"
clean <- survey %>%  
...
A: 310 rows (2 rows of outliers were removed)
...
# See what the distinct values are in the Handed column (Get the column names)
unique(survey$Handed)

# The table function tell you how many data in each of the column
table(survey$Handed)

# This following line applies multiple restrictions to the data set
survey %>%
  filter(Height_cm < 250,
         Handed != "Ambidextrous")

# You can also use %in% c("column") to select the columns you want
survey %>%
  filter(Height_cm < 250,
...
new_data <- survey %>%
  filter(Height_cm < 250,
         Handed %in% c("Left-Handed", "Right-Handed"),
         Region %in% c("MO", "FL")) # %>%
  # select(......) You can also pipe the data that you already piped!!
...
```{r}
vegetarians <- survey %>%
  filter(Vegetarian == "Yes",
          Favorite_Food == "Meat")

dim(vegetarians)

unique(survey$Favorite_Food)

boxplot(clean$Height_cm ~ clean$Favorite_Food)
```
