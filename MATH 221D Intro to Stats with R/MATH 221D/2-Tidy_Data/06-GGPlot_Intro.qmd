...
ggplot(iris, mapping = aes(x = Sepal.Length, y = Sepal.Width, color = Species)) +
  ...
  theme_bw()
# use some themes that already exist in the system to make the graph better
# useful themes: theme_bw (black and white), theme_classic (theme_bw without grids)  
```
...
```{r}
# if you set the scales = "free" in facet_grid(), it automatically fits to the range of data
ggplot(iris, mapping = aes(x = Sepal.Length, y = Sepal.Width, color = Species)) +
  geom_point() +
...
```{r}
# Use "fill" to fill the color of boxplots!
