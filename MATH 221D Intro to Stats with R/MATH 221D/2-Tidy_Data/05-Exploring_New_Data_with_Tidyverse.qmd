...
A:Education, Religiosity_score, Smoking, Alcohol, Lying, and I_live_a_healthy_lifestyle categories.
...
# you can uses hist() to replace histogram()
```
...
boxplot(happiness$Happiness_score)
...
unique(happiness$Punctuality)
table(happiness$Punctuality)

unique(happiness$Only_child)
table(happiness$Only_child)

boxplot(happiness$Happiness_score ~ happiness$Only_child) %>%
filter(happiness$Happiness_score >= 0 && happiness$Happiness_score < 40)
...
unique(happiness$Alcohol)
unique(happiness$Smoking)
favstats(happiness$Religiosity_score)


#"never smoked"   "tried smoking"  "former smoker"  "current smoker" "nunya business"
...
boxplot(lying_data$Happiness_score ~ lying_data$Lying)
#View(lying_data)
...
favstats(lying_data$Happiness_score ~ lying_data$Lying)
...
```{r}
unique(happiness$Smoking)
table(happiness$Smoking)


smoking_use <- happiness %>%
  filter(Happiness_score <= 15,
         Happiness_score >= 0,
         Smoking %in% c("never smoked", "tried smoking", "former smoker", "current smoker")) %>%
  select(Happiness_score, Smoking)


boxplot(smoking_use$Happiness_score ~ smoking_use$Smoking)

```

What conclusions can you make?
A: It seems like the happiness score is a subjective number. Firstly, the median of happiness score among different levels of smokers, drinkers, and people in different categories are very close. Secondly, the Q1 and Q3 numbers are also close. Smoking and drinking could be huge impacts in a person's life. So, the happiness score is more likely a self-awared score instead of an objective indicator.
