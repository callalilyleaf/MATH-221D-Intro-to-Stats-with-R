...
```{r}
glimpse(fifa_heart_attacks)

favstats(fifa_heart_attacks$heart_attacks ~ fifa_heart_attacks$time_period)
# The y values goes first, the x value goes second.
...
: Yes. There are serveral outliers, especially in the World Cup category.

Check to see if the means from **both** groups are normally distributed:

  1. Is n > 30 for both groups? Yes
  2. Create a qqPlot() 
...
: Control (before World Cup) as group 1 and World Cup as group 2 (according to alphabetical order)
...
# Response variable first, then the grouping variable
```
...
Because the p-value is low, I reject the null hypothesis.
...
```{r}
t.test(fifa_heart_attacks$heart_attacks ~ fifa_heart_attacks$time_period, conf.level = 0.97)$conf.int


```

In context of the research question, interpret the confidence interval:
I'm 97% confident that the heart attack frequency, during the World Cup period, is between 2.595060 and 7.317028 percent higher than the normal time in average.
...
```{r}
boxplot(rugby$time ~ rugby$period, main="The Successive Times in Newrules versus Oldrules", col=c(3,4))
```
...
```{r}
favstats(rugby$time ~ rugby$period)%>%knitr::kable()
```
...
: Yes, since the n is bigger than 30 even though the qqPlot graphs look like skewed.

Why is it OK to continue with the analysis?
: Because the n number is big enough.


Perform a t-test.

```{r}
t.test(rugby$time ~ rugby$period, mu = 0, alternative = "greater")
